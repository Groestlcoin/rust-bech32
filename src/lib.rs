...
//! const TAP_ADDR: &str = "grs1p4w46h2at4w46h2at4w46h2at4w46h2atf8lcyc";
//!
...
//! let string = bech32grs::encode::<Bech32m>(hrp, &DATA).expect("failed to encode string");
//! assert_eq!(string, STRING);
//!
//! // Encode arbitrary data as a Groestlcoin taproot address.
//! let taproot_address = segwit::encode(hrp::GRS, segwit::VERSION_1, &DATA).expect("valid witness version and program");
...
//! let (hrp, data) = bech32grs::decode(&STRING).expect("failed to decode");
//! assert_eq!(hrp, Hrp::parse("abc").unwrap());
//! assert_eq!(data, DATA);
//!
//! // Decode a Groestlcoin taproot address.
